...
object OnnxInferenceManagerMulti {
    ...
    // <<<--- 실제 모델 파일명으로 변경하세요! --- >>>
    private const val VIDEO_MODEL_FILE = "IIANetVideoModel.onnx" // 비디오 특징 추출 모델
    private const val AUDIO_MODEL_FILE = "IIANetAudioModel_sim.onnx" // 오디오 처리 모델

    // --- 모델 입/출력 이름 및 형태 정의 (반드시 실제 모델에 맞게 수정!) ---
    // 비디오 모델 (VideoModel)
    private const val VID_INPUT_NAME = "input.1" // 비디오 모델의 비디오 입력 이름 (예시)
    private val vidInputShape = longArrayOf(1L, 1L, 25L, 88L, 88L)
    // 비디오 모델의 출력 이름이자, 오디오 모델의 입력 이름 중 하나
    private const val VID_OUTPUT_EMB_NAME = "247" // 예시, 실제 이름 확인 필수!
    private val vidOutputEmbShape = longArrayOf(1L, 512L, 25L)

    // 오디오 모델 (AudioModel)
    private const val AUD_INPUT_WAV_NAME = "onnx::Squeeze_0" // 오디오 모델의 오디오 입력 이름 (예시)
    private val audInputWavShape = longArrayOf(1L, 1L, 16000L)
    // 오디오 모델의 임베딩 입력 이름 (비디오 모델의 출력 이름과 일치해야 함)
    private const val AUD_INPUT_EMB_NAME = "input.3" // 예시, 실제 이름 확인 필수!
    // private val audInputEmbShape = longArrayOf(1L, 512L, 25L) // 위와 동일

    // 오디오 모델의 최종 오디오 출력 이름
    private const val AUD_OUTPUT_NAME = "11916" // 예시, 실제 이름 확인 필수!
...
//                     try { addNnapi() } catch (e: Exception) { Log.w(TAG, "NNAPI 설정 실패", e) }
                ...
                // 각 모델에 대한 세션 생성
                videoSession = ortEnvironment?.createSession(videoModelBytes, sessionOptions)
                audioSession = ortEnvironment?.createSession(audioModelBytes, sessionOptions)
